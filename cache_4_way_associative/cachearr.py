...
class cachearr():
    ...
    def print_cache(self):
        ...
        #print("index", "  ","valid", "   ", "dirty", "   ", "tag", "     ", "data" )
        for i in range(0,2**self.numsets):
            self.cachebank[i][2].print_cache_line()
    ...
    def load(self, addr):
        ...
        ch_line3 = self.cachebank[ch_set_dec][2]
        ch_line4 = self.cachebank[ch_set_dec][3]
    ...
    def writeback(self,data, Tag, set_dec, addr_bin, i_index):
        ...
        ch_line3= self.cachebank[set_dec][2]
        ch_line4= self.cachebank[set_dec][3]
        ...
        if(ch_line.tag != Tag and ch_line2.tag != Tag and ch_line3.tag != Tag and ch_line4.tag != Tag):
            ...
            if ch_line.tag == Tag:
                ...
                #print("TAG 2")
                self.cachebank[set_dec][2].write_data(data)
                if(self.rep_pol == 2) :
                    #actualiza el dato más usado 
                    self.read_LRU(set_dec, 2)
            elif(ch_line4.tag==Tag):
                #print("TAG 2")
                self.cachebank[set_dec][3].write_data(data)
                if(self.rep_pol == 2) :
                    #actualiza el dato más usado 
                    self.read_LRU(set_dec, 3)
    ...
    def replace_FIFO(self, n_set, Tag):
        ...
        """
        if self.cachebank[n_set][4] == []:
            # Si la lista está vacia por defecto toma la primer poscición 
            self.cachebank[n_set][4].append(0)
            return 0
        elif(len(self.cachebank[n_set][4])==1):
            ...
            if (Tag == self.cachebank[n_set][0].tag):
                ...
                self.cachebank[n_set][4].append(1)
                return 1
        elif(len(self.cachebank[n_set][4])==2):
            # si contiene un solo valor, por defecto se escribe el segundo en
            # segunco bloque. self.cachebank[set_dec][0]
            if (Tag == self.cachebank[n_set][0].tag):
                return 0
            elif (Tag == self.cachebank[n_set][1].tag):
                return 1
            else :
                self.cachebank[n_set][4].append(2)
                return 2
        elif(len(self.cachebank[n_set][4])==3):
            # si contiene un solo valor, por defecto se escribe el segundo en
            # segunco bloque. self.cachebank[set_dec][0]
            if (Tag == self.cachebank[n_set][0].tag):
                return 0
                ...
                return 1
            elif (Tag == self.cachebank[n_set][2].tag):
                return 2
            else :
                self.cachebank[n_set][4].append(3)
                return 3
        else:
            # Remplazar el primero y meterlo a la cola
            # Sirve para LRU 
            n_block = self.cachebank[n_set][4].pop(0)
            self.cachebank[n_set][4].append(n_block)
            return n_block
    ...
    def replace_Rand(self, n_set, Tag):
        ...
        if self.cachebank[n_set][4] == []:
            ...
            n = self.cachebank[n_set][4].index(rn)
            n_block = self.cachebank[n_set][4].pop(n)
            self.cachebank[n_set][4].append(n_block)
            return n_block
    ...
    def read_LRU(self, n_set, i_block):
        ...
        index = self.cachebank[n_set][4].index(i_block) # Busca el indice donde se encuentra
        # el dato utilizado

        # El dato utilizado se pone al final de la cola, por lo tanto el 
        # valor menos usado se encuentra en el inicio de la lista.
        self.cachebank[n_set][4].append(self.cachebank[n_set][4].pop(index))
